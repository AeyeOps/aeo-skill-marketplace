...
Implemented (without cross-lens awareness — see plan).
...
Root cause: `build_extraction_prompt()` in `base.py` injects `<project>/opt/aeo/aeo-skill-marketplace</project>` into `<session_context>`, but neither `LEARNINGS_PROMPT` nor `KNOWLEDGE_PROMPT` references this field or constrains extraction scope to it.
...
8 of 18 scored learnings entries at w >= 0.25 (44%) are declarative facts:
...
Five deficiencies compound to produce the misalignment above.
...
#### 5. No litmus test for borderline entries
...
- Output format enforcement sections in both prompts — strict JSONL constraints are appropriate for Opus
- `suggested_target` hierarchy guidance in the learnings prompt
...
The learnings prompt has the worst misalignment (97% off-project + 44% lens bleed). Five changes, applied to `LEARNINGS_PROMPT` in `aeo-nous/hooks/lenses/learnings.py`:
...
1. "Would this be true regardless of what happened this session?" → YES = knowledge, not learning
...
- Declarative facts in the learnings store drop from ~44% to <10%
